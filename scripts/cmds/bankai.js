...
async function animeSong(api, event, args, message) {
    ...
    try {
        const animeSongResponse = await axios.get("https://bankaii-edit.vercel.app/kshitiz");
        ...
        const writer = fs.createWriteStream(path.join(__dirname, "cache", `bankai.mp4`));
        ...
        writer.on('finish', async () => {
            const audioFile = path.join(__dirname, "cache", "bankai.mp4");
...
module.exports = {
    config: {
        ...
        countDown: 10,
        role: 0,
        shortDescription: "bankai",
        longDescription: "bankai edits",
        category: "anime",
        guide: "{p}bankai"
    },
